#!/usr/bin/env bash
# Spec: specs/001-agentic-harness/spec.md (ported from spec-spine spec 020)
#
...
if [ -z "$BIN" ] && [ -x "$HOME/.cargo/bin/spec-spine" ]; then BIN="$HOME/.cargo/bin/spec-spine"; fi
...
[merge-derived-index] no spec-spine binary found; cannot auto-resolve $PATHNAME.
            Install it (\`make setup\`), then re-run the rebase/merge,
            or resolve manually:
