...
/// (spec 015 B-6). From spec 035 B-3 it counts failed appends only.
...
/// The metric counting decisions still owed when a drain's bound expired
/// (spec 035 B-2).
pub const METRIC_DECISIONS_ABANDONED: &str = "kernel_decisions_abandoned";

/// Why a denied decision did not reach the chain (spec 035 B-3).
///
/// Each cause is a different operator question: a full queue is load, a
/// failed append is the store, and an abandoned record is a stop that ran
/// out of time. One counter for all three answered none of them.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub enum Cause {
    /// The bounded queue refused the record: it was full, or the kernel had
    /// already closed it.
    Dropped,
    /// The append returned an error.
    Failed,
    /// The record was still owed to the appender when a drain's bound
    /// expired, or when the runtime ended the appender.
    Abandoned,
}

impl Cause {
    /// Every cause, in the order a reader meets them.
    pub const ALL: [Self; 3] = [Self::Dropped, Self::Failed, Self::Abandoned];

    /// The stable label: `dropped`, `failed`, or `abandoned`.
    #[must_use]
    ...
    /// The metric this cause raises.
    #[must_use]
    pub const fn metric(self) -> &'static str {
        match self {
            Self::Dropped => METRIC_DECISIONS_DROPPED,
            Self::Failed => METRIC_LEDGER_FAILURES,
            Self::Abandoned => METRIC_DECISIONS_ABANDONED,
        }
    }
}
...
type FailureObserver = Box<dyn Fn(&DecisionId, Cause, &Error) + Send + Sync + 'static>;
...
/// Register an observer invoked when a denied decision does not reach the
/// chain, with the [`Cause`] (spec 035 B-3).
///
...
where
    F: Fn(&DecisionId, Cause, &Error) + Send + Sync + 'static,
{
...
/// Decisions abandoned by a drain whose bound expired (spec 035 B-2).
#[must_use]
...
#[must_use]
pub fn metrics() -> [(&'static str, u64); 4] {
    [
        (METRIC_DECISIONS, decisions()),
        (METRIC_DECISIONS_DROPPED, dropped()),
        (METRIC_DECISIONS_ABANDONED, abandoned()),
...
/// Count and report a decision that did not reach the chain.
pub(crate) fn record_loss(id: &DecisionId, cause: Cause, err: &Error) {
    let counter = match cause {
        Cause::Dropped => &DROPPED,
        Cause::Failed => &LEDGER_FAILURES,
...
/// Hand a loss to the observers, or to stderr when there are none.
fn report(id: &DecisionId, cause: Cause, err: &Error) {
    if let Some(lock) = FAILURE_OBSERVERS.get() {
        let observers = lock.read().unwrap_or_else(PoisonError::into_inner);
        if !observers.is_empty() {
            for observer in observers.iter() {
                observer(id, cause, err);
            }
            return;
        }
    }
    eprintln!(
        "{}: decision {id} was not written to the chain ({cause}): {err}",
        cause.metric()
    );
}
