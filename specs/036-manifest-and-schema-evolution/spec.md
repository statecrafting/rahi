...
- **D-15 (2026-09-19, owner-authorized correction; D-11's census is per
  ...
  **The seam this needs, and its cost.** A deterministic test has to commit
  ...
  function, so `Ledger` carries a hook that is run once inside the chain
  read: `None` on every ledger this crate builds, installed only through
  `Ledger::with_read_interleave`, and inert until it is. It is
  `#[doc(hidden)]` and is named as scaffolding here rather than hidden
  behind a feature, because a feature only this crate's tests enable has to
  be turned on by a dependency on itself, which `cargo package` resolves
  against the registry and fails on for a version not yet published. That
  was tried first and is what the release job refused. The cost is one
  `Option` field and one call that does nothing; the alternative was a
...
- **D-16 (2026-09-19, owner-authorized correction; what legacy restore
  ...
  demonstrate.
- **D-17 (2026-09-24, correction session, owner-authorized work order; reads
  no B-n: a test-support defect).** `free_port` in
  `crates/rahi-cli/tests/evolution.rs` bound `127.0.0.1:0`, read the port,
  ...
  to the evolution tests. The helpers keep their signatures, so no caller
