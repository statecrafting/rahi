...
- **D-6 (2026-09-20, correction session; reads FR-003 and B-2's budget).**
  ...
  enclose a build.
- **D-7 (2026-09-24, correction session, owner-authorized work order; reads
  no B-n: a test-support defect).** `free_port` in `src/boot.rs`, which
  allocates every port a booted cell listens on bound `127.0.0.1:0`, read
  the port, and dropped the listener before hiqlite (or the spawned cell)
  bound it. That is a time-of-check race: under parallel test binaries
  another process could take the port in the gap, and hiqlite then panicked
  in its `start.rs` with `valid RPC socket address: AddrInUse`. hiqlite
  binds the address itself and accepts no listener, so holding the socket
  until the bind is not available. The allocator now hands out ports from
  ...
  to every harness boot. The helpers keep their signatures, so no caller
