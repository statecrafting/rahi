...
- **D-10 (2026-09-24, correction session, owner-authorized work order; reads
  no B-n: a test-support defect).** `free_addr` in `src/lib.rs`'s unit tests
  and in `tests/adjudicate.rs` bound `127.0.0.1:0`, read the port, and
  ...
  to the kernel tests that boot a store. The helpers keep their signatures,
  so no caller changed. Rejected: a shared helper crate or a dev-dependency
  on `rahi-harness`, which would add a dependency edge from a lower-numbered
